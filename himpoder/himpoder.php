<?php
    ...
    class HimPoder extends DB {
        public function todoHimnario(){
            return $con = $this -> conectar() -> query('SELECT * FROM him_poder ORDER BY idcancion ASC');
        }
        ...
        public function existeCancion($id) {
            $con = $this -> conectar() -> prepare('SELECT idcancion FROM him_poder WHERE idcancion=:id');
            $con -> execute([':id' => $id]);
            $count = $con -> rowCount();
            $con = null;
            return $count;
